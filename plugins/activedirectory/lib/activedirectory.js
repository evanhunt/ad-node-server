...
var ActiveDirectory = function(url, baseDN, username, password, defaults) {
    if (this instanceof ActiveDirectory) {
        ...
        if (typeof(url) === 'string') {
            ...
            if (!this.opts.bindDN)
                this.opts.bindDN = this.opts.username;
            if (!this.opts.bindCredentials)
                this.opts.bindCredentials = this.opts.password;
        ...
        log.info(
            'Using username/password (%s/%s) to bind to ActiveDirectory (%s).', this.opts.bindDN, isPasswordLoggingEnabled
            ? this.opts.bindCredentials
            : '********',
        this.opts.url);
        log.info(
            'Referrals are %s', defaultReferrals.enabled
...
function truncateLogOutput(output, maxLength) {
    ...
    if ((!length) || (length < (maxLength + 3)))
        return (output);

    var prefix = Math.ceil((maxLength - 3) / 2);
    var suffix = Math.floor((maxLength - 3) / 2);
    return (output.slice(0, prefix) + '...' + output.slice(length - suffix));
}
...
function isGroupResult(item) {
    ...
    if ((item.objectClass) && (item.objectClass.length > 0)) {
        return (_.any(item.objectClass, function(c) {
            return (c.toLowerCase() === 'group');
        }));
    }
    return (false);
}
...
function createClient(url, opts) {
    ...
    var opts = getLdapClientOpts(_.defaults({}, {
        url: url
    }, opts, this.opts));
...
function getLdapClientOpts(opts) {
    return (_.pick(opts || {},
    // Client
    'url', 'host', 'port', 'secure', 'tlsOptions', 'socketPath', 'log', 'timeout', 'idleTimeout', 'reconnect', 'queue', 'queueSize', 'queueTimeout', 'queueDisable', 'bindDN', 'bindCredentials', 'maxConnections'));
}
...
function getLdapSearchOpts(opts) {
    return (_.pick(opts || {},
    // Search
    'filter', 'scope', 'attributes', 'controls', 'paged', 'sizeLimit', 'timeLimit', 'typesOnly', 'derefAliases'));
}
...
function search(baseDN, opts, callback) {
        ...
        while (referralUrl = (ref.uris || [])[index++]) {
           ...
           */
                    function onReferralError(err) {
                        log.error(err, '[%s] An error occurred chasing the LDAP referral on %s (%j)', (err || {}).errno, referralBaseDn, opts);
   ...
   */
    function onClientError(err, res) {
        ...
        client.unbind();
        log.error(err, '[%s] An error occurred performing the requested LDAP search on %s (%j)', (err || {}).errno || 'UNKNOWN', baseDN, opts);
        if (callback)
            callback(err);
        }
   ...
   */
    function onSearchEnd(result) {
        if ((!pendingRangeRetrievals) && (pendingReferrals.length <= 0)) {
            client.unbind();
            log.info('Active directory search (%s) for "%s" returned %d entries.', baseDN, truncateLogOutput(opts.filter), (results || []).length);
            if (callback)
    ...
    if (!_.any(controls, function(control) {
        return (control instanceof ldap.PagedResultsControl);
    })) {
        log.debug(
            'Adding PagedResultControl to search (%s) with filter "%s" for %j', baseDN, truncateLogOutput(opts.filter), _.any(opts.attributes)
            ? opts.attributes
            : '[*]');
        controls.push(new ldap.PagedResultsControl({
            value: {
    ...
    if (opts.includeDeleted) {
        if (!_.any(controls, function(control) {
            return (control.type === '1.2.840.113556.1.4.417');
        })) {
            log.debug(
                'Adding ShowDeletedOidControl(1.2.840.113556.1.4.417) to search (%s) with filter "%s" for %j', baseDN, truncateLogOutput(opts.filter), _.any(opts.attributes)
                ? opts.attributes
                : '[*]');
    ...
    log.debug(
        'Querying active directory (%s) with filter "%s" for %j', baseDN, truncateLogOutput(opts.filter), _.any(opts.attributes)
        ? opts.attributes
        : '[*]');
...
function parseRangeAttributes(result, opts, callback) {
    ...
    _.each(rangeAttributes, function(rangeAttribute, index) {
        ...
        if (!result[rangeAttribute.attributeName])
            result[rangeAttribute.attributeName] = [];
    ...
    log.debug('Attribute range retrieval specifiers %j found for "%s". Next range: %j', rangeAttributes, result.dn, queryAttributes);
...
function shouldIncludeAllAttributes(attributes) {
    return ((typeof(attributes) !== 'undefined') && ((attributes.length === 0) || _.any(attributes || [], function(attribute) {
        return (attribute === '*');
    })));
}
...
function getRequiredLdapAttributesForGroup(opts) {
    ...
    ], includeGroupMembershipFor(opts, 'group')
        ? ['member']
        : []));
}
...
function getRequiredLdapAttributesForUser(opts) {
    ...
    return (_.union(
        [
        'dn', 'cn'
    ], includeGroupMembershipFor(opts, 'user')
        ? ['member']
        : []));
}
...
function getGroupMembershipForDN(opts, dn, stack, callback) {
    ...
    search.call(self, opts, function(err, results) {
        ...
        async.forEach(results, function(group, asyncCallback) {
            ...
            if (isGroupResult(group)) {
                ...
                getGroupMembershipForDN.call(self, opts, group.dn, groups, function(err, nestedGroups) {
                    ...
                    log.debug('Group "%s" which is a member of group "%s" has %d nested group(s). Nested: %j', group.dn, dn, nestedGroups.length, _.map(nestedGroups, function(group) {
                        return (group.dn);
                    }));
        ...
        , function(err) {
            ...
            groups = _.uniq(_.sortBy(groups, function(group) {
                return (group.cn || group.dn);
...
function getDistinguishedNames(opts, filter, callback) {
    ...
    search.call(self, opts, function(err, results) {
        ...
        log.info('%d distinguishedName(s) found for LDAP query: "%s". Results: %j', results.length, truncateLogOutput(opts.filter), results);
        callback(null, dns);
    });
}
...
function getUserDistinguishedName(opts, username, callback) {
    ...
    getDistinguishedNames.call(self, opts, getUserQueryFilter(username), function(err, dns) {
        ...
        log.info('%d distinguishedName(s) found for user: "%s". Returning first dn: "%s"', (dns || []).length, username, (dns || [])[0]);
        callback(null, (dns || [])[0]);
    });
}
...
function getGroupDistinguishedName(opts, groupName, callback) {
    ...
    getDistinguishedNames.call(self, opts, getGroupQueryFilter(groupName), function(err, dns) {
        ...
        log.info('%d distinguishedName(s) found for group "%s". Returning first dn: "%s"', (dns || []).length, groupName, (dns || [])[0]);
        callback(null, (dns || [])[0]);
    });
}
...
ActiveDirectory.prototype.getUsersForGroup = function getUsersForGroup(opts, groupName, callback) {
     ...
     */
        ...
        if (chunks.length > 1) {
            log.debug('Splitting %d member(s) of "%s" into %d parallel chunks', (group.member || []).length, groupName, chunks.length);
        }
        ...
        async.each(chunks, function getUsersForGroup_ChunkItem(members, asyncChunkCallback) {
            ...
            var localOpts = {
                filter: filter,
                scope: 'sub',
                attributes: joinAttributes((opts || {}).attributes || defaultAttributes.user || [], getRequiredLdapAttributesForUser(opts), ['groupType'])
            };
...
ActiveDirectory.prototype.groupExists = function groupExists(opts, groupName, callback) {
    ...
    self.findGroup(opts, groupName, function(err, result) {
        ...
        log.info(
            '"%s" %s exist.', groupName, (result != null)
            ? 'DOES'
            : 'DOES NOT');
...
ActiveDirectory.prototype.isUserMemberOf = function isUserMemberOf(opts, username, groupName, callback) {
    ...
    self.getGroupMembershipForUser(opts, username, function(err, groups) {
        ...
        var result = _.any(groups, function(item) {
            return (((item.dn || '').toLowerCase() === lowerCaseGroupName) || ((item.cn || '').toLowerCase() === lowerCaseGroupName));
        });
        log.info(
            '"%s" %s a member of "%s"', username, result
            ? 'IS'
            : 'IS NOT',
        groupName);
        callback(null, result);
...
ActiveDirectory.prototype.find = function find(opts, callback) {
    ...
    var localOpts = _.defaults(_.omit(opts || {}, 'attributes'), {
        scope: 'sub',
        attributes: joinAttributes((opts || {}).attributes || [], defaultAttributes.group || [], defaultAttributes.user || [], getRequiredLdapAttributesForGroup(opts), getRequiredLdapAttributesForUser(opts), ['objectCategory'])
    });
    search.call(self, localOpts, function onFind(err, results) {
        ...
        async.forEach(results, function(item, asyncCallback) {
            ...
            log.info('%d group(s), %d user(s), %d other found for query "%s". Results: %j', result.groups.length, result.users.length, result.other.length, truncateLogOutput(opts.filter), result);
   ...
   */
    function searchDeletedObjects(baseDN, opts) {
        search.call(self, baseDN, _.defaults({}, opts, {includeDeleted: true}), function onFind(err, results) {
            ...
            log.info('%d deleted objects found for query "%s". Results: %j', deletedItems.length, truncateLogOutput(localOpts.filter), deletedItems);
...
ActiveDirectory.prototype.findGroup = function findGroup(opts, groupName, callback) {
    ...
    search.call(self, localOpts, function onSearch(err, results) {
        ...
        log.info('%d group(s) found for query "%s". Returning first group: %j', results.length, truncateLogOutput(localOpts.filter), group);
        // Also retrieving user group memberships?
        if (includeGroupMembershipFor(opts, 'group')) {
            getGroupMembershipForDN.call(self, opts, group.dn, function(err, groups) {
                if (err) {
...
ActiveDirectory.prototype.findGroups = function findGroup(opts, callback) {
    ...
    var localOpts = _.defaults(_.omit(opts || {}, 'attributes'), {
        ...
        scope: 'sub',
        attributes: joinAttributes((opts || {}).attributes || defaultAttributes.group || [], getRequiredLdapAttributesForGroup(opts), ['groupType'])
    });
    search.call(self, localOpts, function onSearch(err, results) {
        if (err) {
            if (callback)
...
ActiveDirectory.prototype.findUsers = function findUsers(opts, includeMembership, callback) {
    ...
    var localOpts = _.defaults(_.omit(opts || {}, 'attributes'), {
        ...
        scope: 'sub',
        attributes: joinAttributes((opts || {}).attributes || defaultAttributes.user || [], getRequiredLdapAttributesForUser(opts), ['objectCategory'])
    });
    search.call(self, localOpts, function onSearch(err, results) {
        if (err) {
            if (callback)
...
ActiveDirectory.prototype.authenticate = function authenticate(username, password, callback) {
    var self = this;
    log.trace(
        'authenticate(%j,%s)', username, isPasswordLoggingEnabled
        ? password
        : '********');
    ...
    client.bind(username, password, function(err) {
        ...
        var message = util.format(
            'Authentication %s for "%s" as "%s" (password: "%s")', err
            ? 'failed'
            : 'succeeded',
        self.opts.url,
        username,
...
ActiveDirectory.prototype.getRootDSE = function getRootDSE(url, attributes, callback) {
    ...
    if (!url)
        throw new Error('No url specified for the root DSE. Please specify an ldap url in the following format: "ldap://yourdomain.com:389".');
