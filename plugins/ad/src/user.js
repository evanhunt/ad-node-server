...
module.exports = {
    ...
    async addUser(opts) {
        return new Promise(async (resolve, reject) => {
            ...
            let valid = email && String(email).indexOf('@') === -1
            ...
            this._addObject(`CN=${commonName}`, location, userObject).then(res => {
                ...
                let expirationMethod = passwordExpires === false
                ...
                let enableMethod = enabled === false
                    ? 'disableUser'
                    : 'enableUser';
    ...
    async updateUser(userName, opts) {
        return new Promise((resolve, reject) => {
            ...
            for (const name in opts) {
                if (map[name] !== undefined) {
                    ...
                    let value = name === 'password'
                        ? encodePassword(opts[name])
                        : opts[name];
                    if (key !== 'cn') {
                        if (key === 'sAMAccountName') {
                            later.push({sAMAccountName: value});
                            later.push({uid: value});
                            later.push({userPrincipalName: `${value}@${domain}`});
                        } else {
            ...
            const go = () => {
                ...
                this.setUserProperty(currUserName, next).then(res => {
            ...
            this.findUser(currUserName).then(data => {
                if (opts.commonName !== undefined) {
                    return this.setUserCN(currUserName, opts.commonName);
                }
            }).then(data => {
                let expirationMethod = opts.passwordExpires === false
                    ? 'setUserPasswordNeverExpires'
                    : 'enableUser';
                ...
                let enableMethod = opts.enabled === false
                    ? 'disableUser'
                    : 'enableUser';
    ...
    async findSomeUser(userName, opts) {
        userName = String(userName || '');
        return new Promise(async (resolve, reject) => {
            let cached = this._cache.get('users', userName);
            if (cached) {
            ...
            userName = userName.indexOf('@') > -1
                ...
                : userName;
            // const filter = `(|(userPrincipalName=${userName}@${domain})(sAMAccountName=${userName}))`;
            const filter = `(|(userPrincipalName=${userName}@${domain})(sAMAccountName=*${userName}*)(cn=*${userName}*))`;
            const params = {
                filter,
                includeMembership: ['all'],
                includeDeleted: false
    ...
    async setUserPassword(userName, pass) {
        return new Promise((resolve, reject) => {
            ...
            this._userReplaceOperation(userName, {unicodePwd: encodePassword(pass)}).then(resolve).catch(reject);
        });
    },
    ...
    async setUserPasswordNeverExpires(userName) {
        ...
        return this._userReplaceOperation(userName, {userAccountControl: NEVER_EXPIRES});
    },

    async enableUser(userName) {
        const ENABLED = 512;
        return this._userReplaceOperation(userName, {userAccountControl: ENABLED});
    },

    async disableUser(userName) {
        const DISABLED = 514;
        return this._userReplaceOperation(userName, {userAccountControl: DISABLED});
    },
    ...
    async unlockUser(userName) {
        return this._userReplaceOperation(userName, {lockoutTime: 0});
    },
